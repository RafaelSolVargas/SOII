...
public:
    ...
    template <typename T2>
    Native_Wrapper &operator+=(T2 value)
    {
        _value += T1(value);
        return *this;
    }
    ...
    friend T3 &operator<<(T3 &os, const Native_Wrapper &value)
    {
        os << value._value;
        return os;
    }
...
public:
    ...
    struct Find
    {
        enum
        {
            Result = Tail::template Find<Type, Start, Current + 1>::Result
        };
    };
    ...
    enum
    {
        Polymorphic = (int(Length) != int(Count<Head>::Result))
    };
};
...
// LIST of Templates
template <template <typename T> class... Tn>
class ALIST;
